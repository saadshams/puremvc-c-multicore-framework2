...
void testConstructor() {
    const struct IProxy *proxy = puremvc_proxy_init((struct IProxy *) &(struct Proxy){0}, NULL, NULL);
...
void testNameAccessors() {
    const struct IProxy *proxy = puremvc_proxy_init((struct IProxy *) &(struct Proxy){0}, "TestProxy", NULL);

    // test assertions
    assert(strcmp(proxy->getName(proxy), "TestProxy") == 0);

    const struct IProxy *proxy2 = puremvc_proxy_init((struct IProxy *) &(struct Proxy){0}, NULL, NULL);
...
void testDataAccessors() {
    ...
    const struct IProxy *proxy = puremvc_proxy_init((struct IProxy *) &(struct Proxy){0}, "colors", colors);
...
void testDataReassign() {
    ...
    struct IProxy *proxy = puremvc_proxy_init((struct IProxy *) &(struct Proxy){0}, "colors", colors);
...
void testNotifier() {
    const struct IProxy *proxy = puremvc_proxy_init((struct IProxy *) &(struct Proxy){}, NULL, NULL);
    assert(strcmp(proxy->getName(proxy), PROXY_NAME) == 0);
